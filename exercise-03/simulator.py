...
class Simulator:
    ...
    def reset_map(self, map_ind):
        ...
        self.fre_pos = np.ndarray((self.map_hei * self.map_wid - obs_num, self.state_dim),
                                  int)  # free locations: candidates for bot & tgt
        self.obj_pos = np.ndarray((self.obj_num, self.state_dim),
                                  int)  # keep track of all objects, including bot tgt & obs
    ...
    def get_pob_from_ind(self, y, x):
        ...
        return self.cub_siz * (y - pob_edg), self.cub_siz * (y + pob_edg + 1), self.cub_siz * (
        x - pob_edg), self.cub_siz * (x + pob_edg + 1)
    ...
    def act(self):
        ...
        if bot_pos_new[0] == self.obj_pos[self.tgt_ind][0] and bot_pos_new[1] == self.obj_pos[self.tgt_ind][
            1]:  # reaching tgt
            self.state_reward = 1.
