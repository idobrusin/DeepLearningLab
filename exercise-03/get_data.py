...
states = np.zeros([opt.data_steps, opt.state_size], float)
...
for step in range(opt.data_steps):
    ...
    # save data & label
    states[step, :] = rgb2gray(state.pob).reshape(opt.state_size)
...
# 2. save to disk
print('saving data ...')
np.savetxt(opt.states_file, states, delimiter=',')
np.savetxt(opt.labels_file, labels, delimiter=',')
print("states saved to " + opt.states_file)
print("labels saved to " + opt.labels_file)
